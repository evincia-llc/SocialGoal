...
| Sprint 6 | ~half of "3-5 months" (6-7 block) | ~1h (commit span 17:24-18:28 EDT, 2026-07-24 eve) | EF6 -> EF Core: full 30-table model + per-entity configs, Baseline migration + seed, schema parity through `Database.Migrate()`, mutation-tested drift check, characterization port (suite 12->30). D17. Commit-span figure (excludes the inter-sprint S5-gate-close + D16 advisory PRs); operator wall-clock for the evening block not separately confirmed |
