...
| 5 | Modern .NET 10 host + gating spikes (EF Core mapping, Identity hash compat, one read-only slice) | active | ADR recorded; auth + data approach proven; one production-shaped slice runs. Decision gate cleared (D1, D2 decided 2026-07-23). 2026-07-24: all three spikes PASSED (schema parity via live catalog diff; Identity v2 hash verifies + v3 rehash persisted; slice serves /Goal/Details over TestServer from baseline schema); ADR-001 PROPOSED; modern-ci lane green; Web.Release.config transform proof PASSED (publish-content regression documented, not fixed). PR pending |
